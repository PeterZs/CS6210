...
int main(int argc, char **argv) {
  ...
  /**
   * BUG 1:
   * Need to initialize g_num_prod_lock too.
   */
  pthread_mutex_init(&g_num_prod_lock, NULL); // need to initialize
  ...
  /**
   * BUG 2: 
   * Could NOT join a detached thread, it'll fail.
   * And it will excutes the pthread_exit(NULL), which will end the main thread...
   */
...
void *producer_routine(void *arg) {
  ...
  /**
   * BUG 3:
   * Need mutex here to keep --g_num_prod an atomic operation.
   */
...
void *consumer_routine(void *arg) {
  ...
  /**
   * BUG 4:
   * Can't return a pointer to stack memory of a thread.
   */
